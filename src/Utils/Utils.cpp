...
std::string Utils::keyToString(const int key) {
    ...
    // F1-F12
    if (key >= 112 && key <= 123) {
        return "F" + std::to_string(key - 111);
    }
...
int Utils::stringToKey(const std::string& key) {
    static const std::map<std::string, int> stringToKey = {
        {"TAB", 9},
        {"SHIFT", 16},
        {"CTRL", 17},
        {"ALT", 18},
    ...
    if (key.size() > 1 && key[0] == 'F') {
        int fKeyNumber = std::stoi(key.substr(1));
        if (fKeyNumber >= 1 && fKeyNumber <= 12) {
            return 111 + fKeyNumber;
        }
    }
    // Numbers 0-9
