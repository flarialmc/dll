...
void ModuleScript::onKey(KeyEvent& event)
{
    ...
    if (const auto& script = linkedScript.lock())
    {
        bool cancelled = script->registerCancellableEvent("KeyEvent", event.getKey(),
                                                          static_cast<int>(event.getAction()));
...
{
    if (!isEnabled() || !ScriptManager::initialized || event.getButton() == 0 || event.getAction()
        == 0) return;

    if (const auto& script = linkedScript.lock())
    {
        bool cancelled = script->registerCancellableEvent("MouseEvent",
                                                          static_cast<int>(event.getButton()),
                                                          static_cast<int>(event.getAction()));
        if (cancelled) event.cancel();
    }
}
...
void ModuleScript::onPacketReceive(PacketEvent& event)
{
    ...
    if (const auto& script = linkedScript.lock())
    {
        ...
        {
            auto tpOpt = getTextPacket(event.getPacket());
            if (!tpOpt) return;
            const auto& tp = *tpOpt;
            if (!tp.message.empty() and
            !tp.name.empty()
            )
            {
                std::string msg{tp.message};
                std::string name{tp.name};
                int type = static_cast<int>(0);

                bool ChatReceiveEvent = script->registerCancellableEvent(
                    "ChatReceiveEvent", msg, name, type);
...
void ModuleScript::onPacketSend(PacketSendEvent& event)
{
    ...
    if (const auto& script = linkedScript.lock())
    {
        ...
        if (event.getPacket()->getId() == MinecraftPacketIds::Text)
        {
            ...
            if (pkt.message.empty() || pkt.name.empty())
            {
                ...
                bool ChatSendEvent = script->registerCancellableEvent(
                    "ChatSendEvent", msg, name, static_cast<int>(type));
