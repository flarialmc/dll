...
void PlayerNotifier::loadSettings()
{
	...
	for (const auto& settingPair : settings.settings) {
		...
		if (name.contains("player") && name.contains("Enabled")) {
			++totalPlayers;
		}
	}
...
void PlayerNotifier::settingsRender(float settingsOffset)
{
	...
	for (int i = 0; i < totalPlayers; i++) {
		if (this->settings.getSettingByName<std::string>("player" + FlarialGUI::cached_to_string(i)) != nullptr) {
			...
			this->addButton("Remove Player", "", "Remove", [this, i]() {
				this->settings.deleteSetting("player" + FlarialGUI::cached_to_string(i));
				this->settings.deleteSetting("player" + FlarialGUI::cached_to_string(i) + "Enabled");
				this->saveSettings();
				});
			this->extraPadding();
		}
