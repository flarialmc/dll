...
public:
    ...
    void onEnable() override {
        ...
        if (FlarialGUI::inMenu) {
            FlarialGUI::Notify("To change the position of the Title, Please click " +
    ...
    void onDisable() override {
        ...
        Deafen(this, SetupAndRenderEvent, &MovableTitle::onSetupAndRender)
        if(WinrtUtils::checkAboveOrEqual(21,40)) {
    ...
    void updatePosition(UIControl* control) {
        ...
        if (scaledSize == Vec2<float>{0, 0}) {
            currentSize = PositionUtils::getScreenScaledPos(Vec2<float>{100.f, 30.0f});
            return;
        }
