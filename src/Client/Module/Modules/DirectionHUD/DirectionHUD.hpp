	...
	void onRender(RenderEvent& event) {
		...
		for (int i = 0; i < yawSize; i++) {
			...
			if (getOps<bool>("wrapFade")) {
				...
				if (distanceFromCenter > omittedDistance) for (int i = 0; i < colStrings.size(); i++) cols[i]->a = settings.getSettingByName<float>(colStrings[i] + "Opacity")->value * (1.f - (distanceFromCenter - omittedDistance) / (maxDistance - omittedDistance));
			}
