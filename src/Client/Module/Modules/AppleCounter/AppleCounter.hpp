...
#include "AppleListener.hpp"

class AppleCounter : public Module {

public:

    AppleCounter() : Module("Golden Apple Counter", "Counts how many Golden Apples you have\nin your inventory.",
                            IDR_ARROW_PNG, "") {
        Module::setup();
    };

    void onEnable() override {
        EventHandler::registerListener(new AppleListener("Apple", this));
        Module::onEnable();
    }

    void onDisable() override {
        EventHandler::unregisterListener("Apple");
        Module::onDisable();
    }

    void defaultConfig() override {
        if (settings.getSettingByName<std::string>("text") == nullptr)
            settings.addSetting("text", (std::string) "{value} Golden Apples");
