...
#include "Elements/ToolTipParams.hpp"
#include "Elements/Notification.hpp"
#include "Elements/ImagesClass.hpp"
#include <iostream>
#include <string>
...
void
FlarialGUI::RoundedRect(float x, float y, const D2D_COLOR_F color, const float width, const float height, float radiusX,
                        float radiusY) {
    if (isInScrollView) y += scrollpos;
...
void FlarialGUI::RoundedRectWithImageAndText(int index, float x, float y, const float width, const float height,
                                             ...
                                             const float imageWidth, const float imageHeight, const wchar_t *text) {
...
void FlarialGUI::ColorPickerWindow(int index, std::string &hex, float &opacity, bool &rgb) {
    if (ColorPickers[index].isActive) {
        ...
        while (y <= originalY + shadePickerHeight) {
            ...
            gradientStops[0].position = 0.0f;
            gradientStops[1].color = HSVtoColorF((ColorPickers[index].hueX / hlwidth) * 360, 1.0f,
                                                 1.0f - ((y - originalY) / shadePickerHeight));
            gradientStops[1].position = 1.0f;
        ...
        if (
                ((CursorInEllipse(circleX, circleY, Constraints::SpacingConstraint(0.15f, hexPreviewSize),
...
void FlarialGUI::AllahBlur(float intensity) {
    ...
    if (SwapchainHook::init) {
        ...
        // update blur in 60 fps [MIGHT BE CAUSING NO MENU RENDER]
        // if (elapsed.count() >= 0.0166f ||
        //     intensity < Client::settings.getSettingByName<float>("blurintensity")->value) {
        if (SwapchainHook::queue != nullptr)
