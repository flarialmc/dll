...
/**
 * Enable the swap chain hook and perform initial setup based on DirectX version and GPU settings.
 */
void SwapchainHook::enableHook() {
    ...
    // Hook the DirectX present function based on render type
    ...
    // Detect GPU and apply settings if Intel GPU is found
    ...
    // Check for RTSS hooks and terminate if present
...
/**
 * Hooked present function callback that handles rendering logic for each frame.
 */
HRESULT SwapchainHook::swapchainCallback(IDXGISwapChain3* pSwapChain, UINT syncInterval, UINT flags) {
    ...
    // Check VSync state and trigger reset if changed
    ...
    // Initialize resources or render based on DirectX version
    if (!init) {
        if (queue == nullptr) {
            DX11Init();
        }
    ...
    try {
        ...
        Logger::error("Fail at loading all images: {}", ex.what());
    }
...
/**
 * Initialize resources for DirectX 11 rendering.
 */
...
/**
 * Initialize resources for DirectX 12 rendering.
 */
void SwapchainHook::DX12Init() {
    ...
    if (SUCCEEDED(swapchain->GetDevice(IID_PPV_ARGS(&device))) &&
        ...
        D3D11On12CreateDevice(device,
            ...
            (IUnknown**)&queue, 1, 0, &SwapchainHook::d3d11Device, &context, nullptr);
        ...
        device->CreateDescriptorHeap(&heapDescriptorBackBuffers, IID_PPV_ARGS(&D3D12DescriptorHeap));
        ...
        for (int i = 0; i < bufferCount; i++) {
            ...
            d3d11On12Device->CreateWrappedResource(backBufferPtr, &d3d11_flags,
                ...
                D3D12_RESOURCE_STATE_PRESENT, IID_PPV_ARGS(&D3D11Resources[i]));
            D3D11Resources[i]->QueryInterface(&DXGISurfaces[i]);

            D2D1Bitmaps[i] = nullptr;
            D2D::context->CreateBitmapFromDxgiSurface(DXGISurfaces[i], props, &(D2D1Bitmaps[i]));
...
/**
 * Render the frame using DirectX 11.
 */
...
/**
 * Render the frame using DirectX 12.
 */
void SwapchainHook::DX12Render(bool underui) {
    ...
    if (d3d12DescriptorHeapImGuiRender || SUCCEEDED(d3d12Device5->CreateDescriptorHeap(&descriptorImGuiRender, IID_PPV_ARGS(&d3d12DescriptorHeapImGuiRender)))) {
        ...
        if (!d3d12CommandList)
            d3d12Device5->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, allocator, NULL, IID_PPV_ARGS(&d3d12CommandList));
        if (d3d12CommandList) {
            ...
            if (SUCCEEDED(d3d12Device5->CreateDescriptorHeap(&descriptorBackBuffers, IID_PPV_ARGS(&d3d12DescriptorHeapBackBuffers)))) {
                ...
                if (!initImgui) {
                    ...
                    ImGui::CreateContext();
                    ImGui_ImplWin32_Init(window);
                    ImGui_ImplDX12_Init(d3d12Device5, buffersCounts, DXGI_FORMAT_R8G8B8A8_UNORM, d3d12DescriptorHeapImGuiRender,
...
/**
 * Prepare blur effect for DirectX 11 rendering.
 */
...
/**
 * Prepare blur effect for DirectX 12 rendering.
 */
...
/**
 * Synchronize rendering transformations across frames.
 */
...
/**
 * Set up the blur effect based on intensity settings.
 */
...
/**
 * Manage font loading and ImGui font rebuilding.
 */
...
/**
 * Measure FPS and calculate frame factor for smooth rendering.
 */
...
/**
 * Retrieve the saved back buffer texture.
 */
...
/**
 * Save the back buffer for effects like motion blur.
 */
