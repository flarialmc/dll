...
private:
	...
	static float getGammaCallback(uintptr_t a1) {
        ...
        for (uint16_t i = 3; i < 450; i++) {
            ...
            {
                optionsPtr->setvalue(!ModuleManager::getModule("Render Options")->settings.getSettingByName<bool>("chunkborders")->value);
            }

            if (*translateName == "options.dev_disableRenderSky")
            {
                optionsPtr->setvalue(!ModuleManager::getModule("Render Options")->settings.getSettingByName<bool>("sky")->value);
            }

            if (*translateName == "options.dev_disableRenderWeather")
            {
                optionsPtr->setvalue(!ModuleManager::getModule("Render Options")->settings.getSettingByName<bool>("weather")->value);
            }

            if (*translateName == "options.dev_disableRenderEntities")
            {
                optionsPtr->setvalue(!ModuleManager::getModule("Render Options")->settings.getSettingByName<bool>("entity")->value);
            }

            if (*translateName == "options.dev_disableRenderBlockEntities")
            {
                optionsPtr->setvalue(!ModuleManager::getModule("Render Options")->settings.getSettingByName<bool>("blockentity")->value);
            }

            if (*translateName == "options.dev_disableRenderParticles")
            {
                optionsPtr->setvalue(!ModuleManager::getModule("Render Options")->settings.getSettingByName<bool>("particles")->value);
            }

            if(*translateName == "options.vsync")  if(Client::settings.getSettingByName<bool>("vsync")->value) optionsPtr->setvalue(false);
